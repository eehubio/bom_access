  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        ...
        <div className="topbar-center"><span className="agent-badge"><Bot size={14} />Agent 31</span><span className="topbar-title">BOM 接入与标准化</span></div>
        <div className="topbar-actions"><span className="privacy-badge"><LockKeyhole size={13} />本地优先解析</span>{status === "ready" && <button className="button secondary compact" onClick={reset}><Upload size={14} />新建任务</button>}</div>
      </header>
      <div className="body-shell">
        <nav className="sidebar">
          <div className="nav-label">工作区</div>
          <button className="nav-item active"><FileSpreadsheet size={17} />BOM 标准化<span className="nav-count">1</span></button>
          <button className="nav-item"><FileClock size={17} />历史任务</button>
          <button className="nav-item"><Tags size={17} />字段规则</button>
